<div class="card mt-5">
    ...
    @if ($user_id == request()->user()->id)
        ...
        <div class="card-footer text-body-secondary">
            <a href="{{ route('Bobot::edit', $token) }}" class="btn btn-outline-primary"><i
                    class="bx bx-edit me-1"></i></a>
            <form action="{{ route('Bobot::destroy', $token) }}" method="post" class="d-inline"
                id="deleteForm{{ $token }}">
                @csrf
                @method('DELETE')
                <button type="button" name="delete" class="btn btn-outline-danger" data-bs-toggle="tooltip"
                    ...
                    onclick="showDeleteConfirmationModal('{{ $token }}')">
